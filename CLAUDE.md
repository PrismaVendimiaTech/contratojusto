...
- Start every non-trivial task with `Skill(ps-contexto)`.
- After context load, run `Skill(brainstorming)` once before planning or execution when decisions, tradeoffs, or missing assumptions remain.
- When the next documentation layer is unclear, run `Skill(ps-asistente-wiki)` before choosing a deeper specialist skill.
- Use `Skill("mi-lsp")` first, always, for repository exploration, code navigation, docs-first Q&A, and batch reads. Bootstrap it or report the blocker before broad raw inspection in `packages/`.
- If the task edits `AGENTS.md` or `CLAUDE.md`, use `Skill(ps-crear-agentsclaudemd)`.
- Use `Skill(writing-plans)` when the user explicitly asks for a structured implementation plan after context is loaded.
...
- Use `AskUserQuestion` when it is available in the current mode; otherwise ask directly in chat.
- Run `Skill(ps-trazabilidad)` before closing every non-trivial task.
- Run `Skill(ps-auditar-trazabilidad)` for large, risky, or multi-module changes.
...
1. Run `Skill(ps-contexto)`.
2. Run `Skill(brainstorming)` when open decisions remain.
3. Execute the change.
4. Run `Skill(ps-trazabilidad)`.

### Large, Risky, or Multi-Module Change Flow

1. Run `Skill(ps-contexto)`.
2. Run `Skill(brainstorming)`.
3. If the user explicitly asked for delegation or parallel exploration, split the work into at least 2 disjoint subagents when safe and keep the main agent focused on integration and final verification.
4. Execute and verify.
5. Run `Skill(ps-auditar-trazabilidad)`.
6. Run `Skill(ps-trazabilidad)`.

### AGENTS.md or CLAUDE.md Policy Change Flow

1. Run `Skill(ps-contexto)`.
2. Use `Skill(ps-crear-agentsclaudemd)`.
...
1. Run `Skill(ps-contexto)`.
2. Run `Skill(brainstorming)` if the next decision is still open.
3. If the next documentation layer is unclear, run `Skill(ps-asistente-wiki)`.
4. Execute exactly one recommended specialist skill for the target layer.
5. Run `Skill(ps-trazabilidad)`.

### Frontend UX-to-Code Flow

1. Run `Skill(ps-contexto)`.
2. Run `Skill(brainstorming)` if intent, interaction, or component boundaries are still open.
3. Load the relevant `10_*`, `11-15`, `10_uxui/`, and `07_tech/TECH-FRONTEND-SYSTEM-DESIGN.md` context.
4. If `UXS` changed or no atomic RFC exists for the touched step, run `Skill(crear-ui-rfc)` before writing code.
5. Implement only against `packages/frontend/`.
6. Update validation, audit, and learning docs when user-visible behavior changed.
7. Run `Skill(ps-trazabilidad)`.

### Deployment or Troubleshooting Flow

1. Use `Skill(dokploy-cli)` for Dokploy actions.
2. Use `Skill(ssh-remote)` or direct `ssh turismo "..."` for host-level logs, filesystem checks, Docker inspection, and Traefik troubleshooting.
3. Keep `infra/.env` local and untracked.
4. Run `Skill(ps-trazabilidad)` if deployment work changes repo docs, scripts, env contracts, or operational guidance.

## 4. Canonical Source of Truth

| Artifact | Path | Status |
| --- | --- | --- |
| Governance doc | `CLAUDE.md` | Active |
| Codex governance mirror | `AGENTS.md` | Active peer policy doc |
...
| --- | --- | --- |
| `<GOVERNANCE_DOC>` | `CLAUDE.md` | Active Claude governance doc |
...
## Token/Blockchain Specification

- **Network**: Stellar Testnet (`Test SDF Network ; September 2015`)
- **Contract runtime**: Soroban (WASM)
- **Smart contract language**: Rust (soroban-sdk)
- **Wallet integration**: Stellar Wallets Kit. The current frontend enables the safe built-in modules first; WalletConnect remains a follow-up integration.
- **Token**: USDC on Stellar testnet (`GBUQWP3BOUZX34STELLA5QC35CZUHG4DYUR67MRHR5QJQ5GGDTDSDUUN`)

### USDC Decimal Precision
- Stellar USDC uses **7 decimals** (NOT 18 like Ethereum, NOT 2 like display)
- 1 USDC = 10,000,000 stroops (smallest unit)
- Smart contract amounts are i128 in stroops
- Frontend conversion: `userAmountUSDC * 10_000_000 = contractAmount`
- Example: user deposits "100 dólares" → contract receives `1_000_000_000` (100 * 10^7)

## 6. Local Search Playbook

- Use `Skill("mi-lsp")` as the mandatory first tool for repository exploration.
...
mi-lsp nav multi-read CLAUDE.md:1-220 AGENTS.md:1-220 .docs/wiki/10_uxui/UI-RFC-INDEX.md:1-220 --workspace vendimia-tech --format compact
...
- Run `Skill(ps-trazabilidad)` before closing every non-trivial task that changed docs, code, contracts, states, UX, UI, or deployment guidance.
- Run `Skill(ps-auditar-trazabilidad)` for large, risky, or multi-module changes before final closure.
...
- Prefer `Skill(dokploy-cli)` over Dokploy MCP for redeploys, env updates, domains, and troubleshooting.
- For Claude Code, prefer the installed `dokploy-cli` wrapper under the local skill path, typically `~/.claude/skills/dokploy-cli/scripts/dkp.sh` on POSIX or the matching PowerShell wrapper on Windows if available.
- Prefer `Skill(ssh-remote)` for SSH operations. If it is unavailable, fall back to direct `ssh turismo "..."`.
...
```bash
$DKP doctor
$DKP status "$DOKPLOY_APP_ID"
$DKP redeploy "$DOKPLOY_APP_ID"
```
...
- Use `AskUserQuestion` when it is available in the current mode; otherwise ask directly.
- Use Claude subagents only when the user explicitly asks for delegation or parallel exploration. When that happens and the split is safe, prefer at least 2 disjoint subagents and keep the main agent focused on integration.
- Keep `CLAUDE.md` and `AGENTS.md` fully self-contained and aligned. Do not delegate policy meaning to the other file.
